...
**Testler:** core tam suite **751 yeşil** (`test_home_my_work.py` 11 · `test_saved_views.py` 3 · `test_issues_retired.py` 3); frontend `test/home` 31 · `test/tasks` yeniden yazılan/yeni: `viewRouting`, `viewsSidebar`, `views.model`, `savedViews.integration`, `visualLanguage`; `explorer.integration` kaldırıldı (ağaç `viewsSidebar`'da), `userAxis` Explorer bloğu düştü; tasks + hardening + i18n **434/436** (2 zaman aşımı yük altında — `crud` ve `logTimeWorkItemPicker` izole yeşil), home/shell/time-entry yeşil; lint temiz; vite build OK.

**Canlı doğrulama (hermes-dev):** P3.1–P3.4 `619f09d` ile canlı (P3.2/P3.3 koşuları stale-run guard'a takıldı, son koşu hepsini taşıdı): `/home/my-work|week|team|org` ve `/tasks/key/*` token'sız 401, `/` ve `/work/TASK-1` 200 (SPA), pod logu temiz. **P3.5/P3.6 (`f195051`, CD 35790646944 başarılı):** core + frontend imajları `f195051`; `/views`, `/issues`, `/home/*`, `/tasks/key/*` token'sız 401 (kimlik kapısı önce; `/issues` 410'u testle kanıtlı — JWT uydurulmaz); `/`, `/work/TASK-1`, `/project-management` 200 (SPA); pod logu temiz; şema değişmedi (head 0013, migrate job'u no-op).

**CTO dev geri bildirimi (23.09) — `6f0b434`, `4eaad34`:** takvim yerleşimi `ol` liste sıfırlaması eksikti (40 px boşluk + "1.–7." işaretleri) → düzeltildi, gün sütunları ≥132 px, başlıklar iki satır; ana sayfa 1040 px sınırı kalktı, bloklar akışkan ızgarada satırı dolduruyor, tipografi büyüdü; Time Entry eksik gün kutusu yeniden tasarlandı (yumuşak zemin, tek birincil "+ Log Time", izin küçük bağlantı; jenerik "No logs" tekrarı kalktı). Hepsi hermes-dev'de canlı (`4eaad34`).
